...
// Ceiling for an SGR decimal field. The wire format puts no bound on the
// digit run, so a hostile emulator can send hundreds of them; accumulating
// those into an int32_t overflows, which is undefined behavior (caught by
// the fuzz/fuzz_targets/mouse.cc harness under UBSan). Saturating at 2^24
// keeps every real coordinate and button code exact — no terminal has
// sixteen million columns — while the accumulate step below stays inside
// int32_t for any input.
constexpr int32_t kMaxSgrFieldValue = 1 << 24;
...
bool MouseParser::ParseSgrSequence(const uint8_t* data, size_t length,
  ...
  while (index < length) {
    ...
    if (c >= '0' && c <= '9') {
      has_digit = true;
      // Saturate rather than accumulate past the cap: the guard keeps the
      // operand under 2^24, so the multiply-add below cannot overflow.
      if (values[part] < kMaxSgrFieldValue) {
        values[part] = values[part] * 10 + (c - '0');
        if (values[part] > kMaxSgrFieldValue) {
          values[part] = kMaxSgrFieldValue;
        }
      }
      index += 1;
      continue;
