...
describeIf(os.platform() === 'darwin')('E2E Signature and Cache Tests', () => {
  ...
  // Track cache entries we expect to create, even when the test skips.
  const expectedCacheEntries = new Set<string>()
  // Uncompressed node-smol binary for SEA blob generation, at the stub's version.
  ...
  test(
    'should sign stub and extracted binary on initial injection',
    async () => {
  ...
  test(
    'should create new cache entry and clean old one on overwrite',
    async () => {
  ...
  test(
    'should have valid signatures after multiple overwrites',
    async () => {
      // Clean corrupted cache entries from previous test runs
      await cleanCacheBeforeTest()
