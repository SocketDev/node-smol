...
// setenv() mutates `environ` (glibc/musl reallocate it), so the envp
// captured at main() goes stale and execve(..., envp) drops every
// variable set in this process -- including SMOL_STUB_PATH /
// SMOL_CACHE_KEY, without which the extracted node cannot find the
// VFS payload and degrades to bare node. Exec with the LIVE
// environment instead.
extern char **environ;
