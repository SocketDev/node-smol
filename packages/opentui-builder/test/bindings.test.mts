...
// These exercise the native FFI surface, so they need the Zig build to have
// produced opentui.node / libopentui.dylib. Skip rather than fail when it has
// not: the artifact is build output and is not committed.
const nativeBuilt = isNativeModuleBuilt()

// ── Module Loading ──

describe.skipIf(!nativeBuilt)('opentui native module', () => {
...
describe.skipIf(!nativeBuilt)('RGBA', () => {
...
describe.skipIf(!nativeBuilt)('constants', () => {
...
describe.skipIf(!nativeBuilt)('renderer', () => {
  let renderer
...
describe.skipIf(!nativeBuilt)('optimized buffer', () => {
  let buffer
...
describe.skipIf(!nativeBuilt)('text buffer', () => {
  let tb
...
describe.skipIf(!nativeBuilt)('text buffer view', () => {
  let tb
  let view
...
describe.skipIf(!nativeBuilt)('edit buffer', () => {
  let eb
...
describe.skipIf(!nativeBuilt)('editor view', () => {
  let eb
  let view
...
describe.skipIf(!nativeBuilt)('syntax style', () => {
  let style
...
describe.skipIf(!nativeBuilt)('links', () => {
  afterEach(() => {
...
describe.skipIf(!nativeBuilt)('hit grid', () => {
  let renderer
...
describe.skipIf(!nativeBuilt)('buffer drawing integration', () => {
...
describe.skipIf(!nativeBuilt)('encodeText', () => {
...
const hasDirectBufferAPIs =
  nativeBuilt && typeof native.bufferGetCharArrayBuffer === 'function'
...
const hasCursorIntoAPIs =
  nativeBuilt && typeof native.editBufferGetCursorInto === 'function'
