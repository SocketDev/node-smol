...
describe.skipIf(!existsSync(BINJECT_BIN) || process.platform === 'win32')(
  ...
  () => {
    it(
      'should inject SEA using write_with_notes()',
      async () => {
    ...
    it(
      'should preserve PT_NOTE segment in output binary',
      async () => {
        if (IS_LINUX) {
          // Linux: use readelf
    ...
    it(
      'should correctly handle ALLOC flags for VirtAddr=0 sections (Linux only)',
      async () => {
        if (!IS_LINUX) {
    ...
    it(
      'should produce binary that executes without segfault',
      async () => {
