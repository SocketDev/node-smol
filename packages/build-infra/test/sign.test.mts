 ...
 *   `../lib/sign.mts`. `selectCodesignArgs` takes no filesystem or process
 *   action, so its identity precedence (explicit > env > literal default)
 *   and flag composition (hardened runtime, entitlements) are fully
 *   verifiable without a real certificate or `codesign` invocation.
 */
...
import { selectCodesignArgs } from '../lib/sign.mts'
