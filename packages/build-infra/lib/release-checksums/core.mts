 ...
 * bridges it to the SRI pin via `@socketsecurity/lib/crypto/integrity`.
 *
...
import type { Hash, HashAlgorithm } from '@socketsecurity/lib/crypto/integrity'
import { equalHashes, parseHash } from '@socketsecurity/lib/crypto/integrity'
 ...
 * loads into memory. Defaults to sha256 — the `checksums.txt` / `shasum -a 256`
 * digest. `@socketsecurity/lib/crypto/integrity` has no streaming primitive
 * (its one-shot `computeHash` docs itself defer chunked input back to
