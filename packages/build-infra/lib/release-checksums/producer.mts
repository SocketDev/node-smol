 ...
 *
 * `writeChecksumsFile` writes sha256-hex — `checksums.txt` stays the ecosystem
 * `shasum -c` transport format. `updateReleaseAssets` re-encodes that same hex
 * map to SRI (`@socketsecurity/lib/crypto/integrity`'s `parseHash`) before
 * embedding it as the `release-assets.json` pin; a caller that already hands it
 * an SRI string is untouched (`parseHash` is idempotent on SRI input).
 *
...
export function updateReleaseAssets(config: UpdateAssetsOptions): void {
  ...
  let manifest: EmbeddedChecksums & {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- null-proto idiom: `__proto__: null` keeps the seed prototype-free but changes its inferred type, so the cast is what makes the declared shape hold.
    { __proto__: null } as unknown as EmbeddedChecksums & {
      $schema?: string | undefined
      $comment?: string | undefined
    }
  try {
  ...
  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- null-proto idiom: `__proto__: null` keeps the map prototype-free but changes its inferred type, so the double cast is required.
  const sriChecksums = { __proto__: null } as unknown as Record<string, string>
