...
export async function runBinaryTest(
  ...
  function handleRunError(e, execPath: string): boolean {
  ...
  try {
    ...
    const execPath = WIN32 ? path.win32.normalize(binaryPath) : binaryPath
    return handleRunError(e, execPath)
  }
}
...
export async function smokeTestBinary(
  ...
  async function runNativeSmokeTests(): Promise<boolean> {
    ...
    const simpleTestFile = path.join(TEST_FIXTURES_DIR, 'smoke-test-simple.mjs')
    if (
      !(await runBinaryTest(
        binaryPath,
        [simpleTestFile],
    ...
    if (
      !(await runBinaryTest(binaryPath, [modulesTestFile], 'Built-in modules'))
    ) {
      return false
    }
  ...
  async function runCrossCompileStrategy(strategy): Promise<boolean> {
  ...
  async function selectSmokeTestStrategy(hostArch, hostPlatform) {
    const libc =
      options.libc ||
    ...
    return selectCrossCompileSmokeTestStrategy({
      hasDocker,
      hasQemu,
      hasRosetta,
      hostArch,
  ...
  try {
    ...
    if (!crossCompiled) {
      return runNativeSmokeTests()
    }

    // Cross-compiled binaries: determine test method
    logger.log('  Cross-compiled binary detected')

    const strategy = await selectSmokeTestStrategy(hostArch, hostPlatform)

    return runCrossCompileStrategy(strategy)
  } catch (e) {
