 ...
 *
 *   Usage: node scripts/fleet/check/fleet-pack-workflow-payloads-are-fetchable.mts [--quiet]
 */
...
export function main(): number {
  ...
  if (!isFleetPackConsumer(REPO_ROOT)) {
    if (!quiet) {
      logger.success(
        '[fleet-pack-workflow-payloads-are-fetchable] not a thin member (no bundle.ref) — the payload fetch is a no-op here.',
      )
    }
    return 0
  }
  ...
  if (unwired.length) {
    logger.fail(
      '[fleet-pack-workflow-payloads-are-fetchable] a thin member has workflow step(s) that cannot authenticate the bundle fetch:',
    )
    logger.error(
  ...
  if (!quiet) {
    logger.success(
      `[fleet-pack-workflow-payloads-are-fetchable] all fleet install/checkout steps across ${workflows.length} workflow(s) can mint a payload token.`,
    )
  }
  return 0
}
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/fleet-pack-workflow-payloads-are-fetchable.mts`,
}
