#!/usr/bin/env node
/*
 * @file `check --all` gate: the fleet-pack-distribution untrack set NEVER contains a
 *   CI-critical GitHub path. A thin member `git rm --cached`s every path
 *   `fleetPackOwnedPaths` returns; GitHub reads `.github/workflows/**` (a
 ...
 *   `fleetPackOwnedPaths`; this check proves the SHIPPED fetcher
 *   (scripts/repo/bootstrap/fleet.mjs) honors it.
 *
 *   The proof feeds `fleetPackOwnedPaths` a synthetic release manifest whose files
 ...
 *
 *   Usage: node scripts/fleet/check/fleet-pack-ci-files-are-tracked.mts [--quiet]
 */
...
// The shipped dep-0 fetcher whose fleetPackOwnedPaths computes the untrack set.
...
// control fleetPackOwnedPaths MUST return, proving the function ran and the
// result isn't vacuously empty.
const CONTROL_FLEET_PATH =
...
export async function main(): Promise<void> {
  ...
  if (!existsSync(fetcherPath)) {
    if (!quiet) {
      logger.log(
        `fleet-pack-ci-files-are-tracked: no ${FLEET_FETCHER_REL} — vacuous pass.`,
      )
    }
    process.exitCode = 0
    return
  ...
  const { fleetPackOwnedPaths } = (await import(
    pathToFileURL(fetcherPath).href
  )) as {
    fleetPackOwnedPaths: (manifest: {
  ...
  const untrack = fleetPackOwnedPaths({ files })
  ...
  if (offenders.length === 0 && controlPresent) {
    if (!quiet) {
      logger.log(
        `fleet-pack-ci-files-are-tracked: ${ciPaths.length} CI path(s) stay tracked; untrack set is CI-safe.`,
      )
    }
    process.exitCode = 0
    return
  ...
  if (!controlPresent) {
    logger.fail(
      'fleet-pack-ci-files-are-tracked: the control path did not appear in the untrack set — the check could not verify exclusion.',
    )
    logger.fail(
      '  What:  fleetPackOwnedPaths returned an unexpected result (empty or\n' +
        '         missing the wholly-fleet control path) — a broken import or a\n' +
        '         regressed function would false-green the CI assertion below.\n' +
        `  Where: ${FLEET_FETCHER_REL} (fleetPackOwnedPaths).\n` +
        `  Wanted: ${CONTROL_FLEET_PATH} in the untrack set.\n` +
        '  Fix:   regenerate the fetcher (node scripts/repo/gen/bootstrap.mts) and\n' +
        '         confirm fleetPackOwnedPaths is exported + intact.',
    )
    process.exitCode = 1
    return
  }
  ...
  logger.fail(
    `fleet-pack-ci-files-are-tracked: ${offenders.length} CI path(s) are in the thin untrack set:`,
  )
  ...
  logger.fail(
    '  What:  a CI-critical GitHub path is in the thin untrack set. A thin\n' +
      ...
      '  Fix:   fleetPackOwnedPaths must skip isAlwaysTrackedGitHubSurface(p)\n' +
...
const SCRIPT_META: ScriptMeta = {
  describe:
    'checks the fleet-pack-distribution untrack set never removes a CI-critical GitHub path',
  help: `Usage: node scripts/fleet/check/fleet-pack-ci-files-are-tracked.mts [flags]
