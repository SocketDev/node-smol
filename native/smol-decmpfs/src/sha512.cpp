// SHA-512 (FIPS 180-4). Straightforward reference implementation; correctness
// over speed — the reader hashes one payload per addon, off any hot path.
#include "sha512.h"
...
void update(Ctx& c, const uint8_t* data, size_t len) {
  c.len += len;
  if (c.buf_len > 0) {
    while (len > 0 && c.buf_len < 128) {
      c.buf[c.buf_len++] = *data++;
      --len;
    }
    if (c.buf_len == 128) {
      block(c, c.buf);
...
void finish(Ctx& c, uint8_t out[64]) {
  uint64_t total_bits = c.len * 8;
  uint8_t pad = 0x80;
  update(c, &pad, 1);
  uint8_t zero = 0;
  while (c.buf_len != 112) {
  ...
  // SHA-512 uses a 128-bit length field; messages here are far under 2^64
  // bytes, so the high 64 bits are always zero.
  uint8_t lenbuf[16];
  std::memset(lenbuf, 0, 8);
  for (int i = 0; i < 8; ++i) {
    lenbuf[8 + i] = uint8_t(total_bits >> (56 - i * 8));
  }
  update(c, lenbuf, 16);
  for (int i = 0; i < 8; ++i) {
