...
GitHub Actions suppresses every event created with the default `GITHUB_TOKEN` — pushes, `pull_request` open/close/reopen, issue events, tag creation. The only events that still fire are `workflow_shared` and `repository_shared`. This is a hardcoded platform behavior that prevents a workflow from recursively triggering itself.
...
- `workflow_shared` / `repository_shared` from the same job — these are the two events `GITHUB_TOKEN` is allowed to raise, so a dispatch-driven downstream workflow is the supported chaining mechanism.
