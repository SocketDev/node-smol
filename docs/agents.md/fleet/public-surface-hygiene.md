...
`workflow_shared.inputs` keys are kebab-case (`dry-run`, `build-mode`); snake_case silently fails the bypass.
...
- A workflow can't use the default `GITHUB_TOKEN` to trigger another workflow (push / PR / issue events it creates are suppressed; only `workflow_shared` / `repository_shared` fire). Full failure modes + the PAT / dispatch workarounds in [`github-token-limitations.md`](github-token-limitations.md).
